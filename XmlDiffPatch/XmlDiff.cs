...
namespace Microsoft.XmlDiffPatch
{
    public class XmlDiff
    {
        ...
        {
            sourceReader = new XmlTextReader(sourceFile)
            {
                XmlResolver = null
            };
            changedReader = new XmlTextReader(changedFile)
            {
                XmlResolver = null
            };
        }
          ...
          ref XmlReader changedReader)
        {
            ...
            try
            {
                ...
                sourceReader = new XmlTextReader(fileStream1, XmlNodeType.Element, context1)
                {
                    XmlResolver = null
                };
                changedReader = new XmlTextReader(fileStream2, XmlNodeType.Element, context2)
                {
                    XmlResolver = null
                };
            }
        ...
        private void MatchIdenticalSubtrees()
        {
            ...
            queue1.Enqueue(this._sourceDoc);
            queue2.Enqueue(this._targetDoc);
                ...
                for (var index = 0; index < count1; ++index)
                {
                    ...
                    if (xmlDiffParentNode.HasChildNodes)
                    {
                        ...
                        while (xmlDiffNode1 != null)
                        {
                            ...
                            var nodeListHead = (XmlDiffNodeListHead)hashtable1[xmlDiffNode1.HashValue];
                            ...
                            else
                            {
                                ...
                                xmlDiffNode1 = xmlDiffNode1._nextSibling;
                                for (var nextSibling = xmlDiffNode2._nextSibling; xmlDiffNode1 != null && nextSibling != null && ( nextSibling.NodeType != XmlDiffNodeType.ShrankNode && ( this.IdenticalSubtrees(nextSibling, xmlDiffNode1) && hashtable1.Contains(nextSibling.HashValue) ) ); xmlDiffNode1 = xmlDiffNode1._nextSibling)
                                {
                ...
                for (var index = 0; index < count2; ++index)
                {
                    ...
                    if (xmlDiffParentNode.HasChildNodes)
                    {
                        ...
                        while (xmlDiffNode1 != null)
                        {
                            ...
                            {
                                var xmlDiffNode2 = (XmlDiffNode)null;
                                var nodeListHead = (XmlDiffNodeListHead)hashtable2[xmlDiffNode1.HashValue];
                                if (nodeListHead != null)
                                    xmlDiffNode2 = this.HTFindAndRemoveMatchingNode(hashtable2, nodeListHead, xmlDiffNode1);
                                if (xmlDiffNode2 == null || xmlDiffNode1.NodeType < XmlDiffNodeType.None)
                                {
                                    if (xmlDiffNode1.HasChildNodes)
                                        queue1.Enqueue(xmlDiffNode1);
                                    else
                                        xmlDiffNode1._bExpanded = true;
                                    xmlDiffNode1 = xmlDiffNode1._nextSibling;
                                }
                                else
                                {
                                    ...
                                    xmlDiffNode1 = xmlDiffNode1._nextSibling;
                                    for (var nextSibling = xmlDiffNode2._nextSibling; xmlDiffNode1 != null && nextSibling != null && ( nextSibling.NodeType != XmlDiffNodeType.ShrankNode && ( this.IdenticalSubtrees(xmlDiffNode1, nextSibling) && hashtable1.Contains(xmlDiffNode1.HashValue) ) ) && hashtable2.Contains(nextSibling.HashValue); nextSibling = nextSibling._nextSibling)
                                    {
                                        this.HTRemoveNode(hashtable1, xmlDiffNode1);
        ...
        {
            var diffNodeListHead = (XmlDiffNodeListHead)hashtable[node.HashValue];
            if (diffNodeListHead == null)
                return false;
        ...
        private void HTRemoveDescendants(Hashtable hashtable, XmlDiffNode parent)
        {
            ...
            while (true)
            {
                for (; !node._bExpanded || !node.HasChildNodes; node = node._nextSibling)
                {
                    ...
                    for (; node._nextSibling == null; node = node._parent)
                    {
          ...
          XmlDiffNode lastNextSibling)
        {
            ...
            xmlDiffNode2._nextSibling = null;
